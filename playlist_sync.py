...
async def sync_playlist(mapping, navi_api, local_api):
    ...
    if songs_to_remove:
        print(
            f"  -> 发现 {len(songs_to_remove)} 首已移除歌曲，正在从 Navidrome 删除..."
        )
    ...
    for i in range(len(target_ordered_ids) - 2, -1, -1):
        ...
        try:
            ...
            if current_index >= target_index:
                print(
                    f"  - 正在移动歌曲 '{track_to_move}' 到 '{insert_before_id}' 的前面..."
                )
                if not await navi_api.move_track_in_playlist(
                    navidrome_id, track_to_move, insert_before_id
                ):
                    print("  - ✗ 移动歌曲失败，排序中断。")
                    break
                # 更新本地顺序以反映移动
    ...
    local_api.update_sync_time(navidrome_id)
    print(
        f"✓ 同步完成！新增 {len(songs_to_add)} 首，移除 {len(songs_to_remove)} 首，并已更新顺序。"
    )
...
async def main():
    ...
    parser.add_argument(
        "--navidrome-url", required=True, help="您的Navidrome服务地址。"
    )
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, platform, online_playlist_id, navidrome_playlist_id, playlist_name, last_sync_time FROM playlist_mappings"
    )
    ...
    for m in all_mappings:
        ...
        if sync_time_str:
            try:
                sync_time = datetime.datetime.fromisoformat(sync_time_str).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            except:
                sync_time = "格式错误"
        print(
            f"  ID: {m[0]} | 平台: {m[1]:<8} | 名称: {m[3]:<30} | 上次同步: {sync_time}"
        )
