...
class NavidromeAPI:
    ...
    async def login(self):
        ...
        try:
            ...
            self.subsonic_token, self.subsonic_salt = (
                data.get("subsonicToken"),
                data.get("subsonicSalt"),
            )
    ...
    async def search_song(self, artist, title, album=None):
        ...
        try:
            ...
            if album_lower:
                for song in results:
                    if (
                        song.get("artist", "").lower() == artist_lower
                        and song.get("title", "").lower() == title_lower
                        and song.get("album", "").lower() == album_lower
                    ):
                        return song.get("id")
            for song in results:
                if (
    ...
    async def add_songs_to_playlist(self, playlist_id, song_ids):
        """异步将歌曲批量添加到歌单。"""
        if not self.is_logged_in or not song_ids:
            return False
    ...
    async def remove_songs_from_playlist(self, playlist_id, track_ids_to_remove):
        """异步从歌单中批量移除歌曲。"""
        if not self.is_logged_in or not track_ids_to_remove:
            return False
        url = f"{self.base_url}/api/playlist/{playlist_id}/tracks"
    ...
    async def move_track_in_playlist(
        self, playlist_id, track_to_move_id, insert_before_id
    ):
        """异步移动歌单中的一首歌曲到另一个位置。"""
        if not self.is_logged_in:
            return False
