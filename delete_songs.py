...
def list_all_songs(cursor):
    """查询并打印数据库中的所有歌曲。"""
    cursor.execute(
        "SELECT id, search_key, album, quality, file_path FROM songs ORDER BY id"
    )
    ...
    print("--- 本地音乐库歌曲列表 ---")
    print(
        f"{'ID':<5} | {'Search Key':<40} | {'Album':<30} | {'Quality':<8} | {'File Path':<50}"
    )
    print("-" * 140)

    for song in songs:
        search_key = (
            (song[1] or "")[:38] + ".." if len(song[1] or "") > 40 else (song[1] or "")
        )
        album = (
            (song[2] or "")[:28] + ".." if len(song[2] or "") > 30 else (song[2] or "")
        )
        file_path = (
            (song[4] or "")[:48] + ".." if len(song[4] or "") > 50 else (song[4] or "")
        )

        print(
            f"{song[0]:<5} | {search_key:<40} | {album:<30} | {song[3]:<8} | {file_path:<50}"
        )
...
def main(db_file):
    ...
    if not os.path.exists(db_file):
        print(
            f"错误: 数据库文件 '{db_file}' 不存在。请确保脚本在正确的目录下运行，或使用 --db 参数指定路径。"
        )
        return
    ...
    # 查找并确认要删除的歌曲
    cursor.execute(
        f"SELECT id, search_key, file_path FROM songs WHERE id IN ({','.join('?' for _ in ids_to_delete)})",
        ids_to_delete,
    )
