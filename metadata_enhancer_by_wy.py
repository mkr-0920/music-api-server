...
def enhance_metadata(target_directory, dry_run=False, start_time=None):
    ...
    all_music_files = [file for file in music_path.rglob('*') if file.suffix.lower() in ['.mp3', '.flac']]
    ...
    if args.since:
        try:
            ...
            try:
                ...
                print(f"错误: 无效的时间格式 '{args.since}'。请使用 'YYYY-MM-DD' 或 'YYYY-MM-DD HH:MM:SS'。")
                sys.exit(1)
        print(f"\n--- 时间过滤器已启用：只处理 {start_time_obj.strftime('%Y-%m-%d %H:%M:%S')} 之后修改的文件 ---")

    if args.directory:
        print(f"--- 扫描指定目录: {args.directory} ---")
        enhance_metadata(target_directory=args.directory, dry_run=args.dry_run, start_time=start_time_obj)
    else:
        ...
        if hasattr(Config, 'MUSIC_DIRECTORY') and Config.MUSIC_DIRECTORY:
            ...
            enhance_metadata(target_directory=Config.MUSIC_DIRECTORY, dry_run=args.dry_run, start_time=start_time_obj)
            scanned_any = True
        if hasattr(Config, 'FLAC_DIRECTORY') and Config.FLAC_DIRECTORY:
            print(f"\n[2/2] 开始扫描FLAC备用目录: {Config.FLAC_DIRECTORY}")
            enhance_metadata(target_directory=Config.FLAC_DIRECTORY, dry_run=args.dry_run, start_time=start_time_obj)
            scanned_any = True
        if not scanned_any:
