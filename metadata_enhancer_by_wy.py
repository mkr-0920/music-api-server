        ...
        if path_obj.suffix.lower() == ".mp3":
            ...
            for key, tag_name in METADATA_MAP["mp3"].items():
                if isinstance(tag_name, tuple):
                    ...
                    value = next(
                        (t.text[0] for t in txxx_tags if t.desc == tag_name[1]), None
                    )
                else:
...
def write_metadata(file_path, data_to_write):
    ...
    try:
        ...
        if path_obj.suffix.lower() == ".mp3":
            ...
            audio.save()
        print(
            f"  - [成功] 已写入 {len(data_to_write)} 项新元数据: {list(data_to_write.keys())}"
        )
        return True
    except Exception as e:
...
def enhance_metadata(target_directory, dry_run=False, start_time=None):
    """【带时间过滤器】按需从网易云百科接口补全缺失的元数据。"""
    print("正在初始化网易云API...")
    try:
    ...
    total_files, processed_files, updated_files = 0, 0, 0
    details_cache = {}
    ...
    def find_best_match(local_artist, local_title, results):
        ...
        for song in results:
            ...
            if (
                local_artist_norm in netease_artist
                and local_title_norm in netease_title
            ):
                return song
        return None
    ...
    for file in all_music_files:
        ...
        if start_time:
            file_mtime_ts = file.stat().st_mtime
            file_mtime_dt = datetime.datetime.fromtimestamp(file_mtime_ts)
            if file_mtime_dt < start_time:
                continue  # 如果文件比指定时间旧，则直接跳过

        processed_files += 1
        print(f"\n[{processed_files}/{total_files}] 正在处理: {file.name}")
        ...
        if not missing_fields and (
            existing_data.get("genre") and existing_data.get("genre").lower() != "pop"
        ):
            print("  - 所有元数据均已存在，跳过。")
            continue

        search_artist = artist
        if "G.E.M.邓紫棋" in artist:
            search_artist = "邓紫棋"
        if "周杰伦" in artist:
        ...
        if netease_details is None:
            ...
            details_cache[search_keyword] = netease_details
            time.sleep(1.5)
        else:
        ...
        if netease_genre and (
            not existing_data.get("genre")
            or existing_data.get("genre").lower() == "pop"
        ):
            if not (
                existing_data.get("genre")
                and existing_data.get("genre").lower() == "pop"
                and netease_genre.lower() == "pop"
            ):
        ...
        if data_to_write:
            if dry_run:
                print(
                    f"  - [试运行] 将会写入 {len(data_to_write)} 项新元数据: {data_to_write}"
                )
                updated_files += 1
            else:
    ...
    print(
        f"\n扫描完成！共找到 {total_files} 个音乐文件，处理了 {processed_files} 个符合时间条件的文件，更新了 {updated_files} 个文件。"
    )
...
if __name__ == "__main__":
    ...
    if args.since:
        try:
            start_time_obj = datetime.datetime.strptime(args.since, "%Y-%m-%d %H:%M:%S")
        except ValueError:
            try:
                start_time_obj = datetime.datetime.strptime(args.since, "%Y-%m-%d")
            except ValueError:
                print(
                    f"错误: 无效的时间格式 '{args.since}'。请使用 'YYYY-MM-DD' 或 'YYYY-MM-DD HH:MM:SS'。"
                )
                sys.exit(1)
        print(
            f"\n--- 时间过滤器已启用：只处理 {start_time_obj.strftime('%Y-%m-%d %H:%M:%S')} 之后修改的文件 ---"
        )
    ...
    if args.directory:
        ...
        if hasattr(Config, "FLAC_DIRECTORY") and Config.FLAC_DIRECTORY:
            ...
            enhance_metadata(
                target_directory=Config.FLAC_DIRECTORY,
                dry_run=args.dry_run,
                start_time=start_time_obj,
            )
            scanned_any = True
